...
//
// $Id: davisamd.cc,v 1.2 2000/02/18 01:31:54 kumfert Exp $
//
...
int main(int argc, char *argv[]) {
  ...
  for ( int iteration = 0; iteration < nIterations; ++iteration ) {
    ...
    const int nEdges = c_graph->queryNEdges();
    const int nNonZeros = c_graph->queryNNonZeros();
  ...
  //
    int n = nVtxs;
    int * pe = new int[ nVtxs+1 ];
    int pfree = nVtxs + 2 * nEdges;
    int * len = new int [ nVtxs ];
    int iwlen = ( pfree * 1.2 > pfree + n ) ? int((pfree*1.2)) : (pfree+n) ;
    int * iw = new int[ iwlen ];
    int * nv = new int[ nVtxs ];
    int * next = new int[ nVtxs ];
    int * last = new int[ nVtxs ];
    int * head = new int[ nVtxs ];
    int * elen = new int[ nVtxs ];
    int * degree = new int[ nVtxs ];
    int * w = new int[ nVtxs ];
    int ncmpa = 0;
    int iovflo = 100000;
  
    /* load up pe with information */
    memcpy( pe, c_graph->getAdjHead().lend(), (nVtxs+1) * sizeof(int ) );
    for ( int k = 0; k<=nVtxs; ++k ) { 
      ++pe[k];
    }
  
    /* load up iw with information */
    memcpy( iw, c_graph->getAdjList().lend(), nNonZeros * sizeof(int) );
    for ( int k = 0; k<nNonZeros; ++k ) { 
      ++iw[k];
    }
    for ( int k = nNonZeros; k<iwlen; ++k ) { 
      iw[k]=0;
    }
    /* Now do len */
    for( int i=0; i<n; ++i ) { 
    ...
    stopwatch timer;
    timer.start();
    amdbar_( &n, pe, iw, len, &iwlen, &pfree, nv, next, last, head, elen, degree, &ncmpa, w, &iovflo);
    timer.stop();
    ...
    perm.getOld2New().import( elen, nVtxs );
    perm.validate();
    if (! perm.isValid() ) { 
